...
lstm_model = load_model('./saved_models/lstm_sentiment_model.h5')

# Adding a progress bar to the prediction process
print("Making predictions with the LSTM model...")
y_pred_prob = np.array([lstm_model.predict(np.expand_dims(X_padded[i], axis=0)) for i in tqdm(range(X_padded.shape[0]))])
